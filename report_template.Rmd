...
## Is gender a significant determinant of being a successful applicant

### Data Manipulation

### Exploratory plots/summary tables

Table 1: Conditional probability table for phase-I candidates
...
prop_t1 <- prop.table(table1, margin = 2)
colnames(prop_t1) <- c("Does not pass", "Pass")
knitr::kable(prop_t1)
```

Table 2: Conditional probability table for phase-II candidates
...
prop_t2 <- prop.table(table2, margin = 2)
colnames(prop_t2) <- c("Does not pass", "Pass")
knitr::kable(prop_t2)
```

Table 3: Conditional probability table for phase-III candidates
...
table3 <- table(hire_2_3$gender, hire_2_3$pass_final)
prop_t3 <- prop.table(table3, margin = 2)
colnames(prop_t3) <- c("Does not pass", "Pass")
...
Before we officially fit models, we build proportion tables to show the percentage of new graduates applicants who admitted to the next round. From table 1, conditional probability table for phase-I candidates, there is no obvious difference between the passing rate of males and females (males: 48.33%, females: 50.67%), therefore about half of the candidates go to the next round (phase-II). For table 2 and table 3, a significant difference of the passing rate is exhibited here.  The passing rate for male applicants is more than twice that for female applicants; 68.18% males in the first round successfully go into the final interview, while there is only 31.81% females. The final round, which decides who gets the job, shows an even larger difference between the passing rate. Here, 80% of the males in the last get the job offer, where only 20% females get in eventually. Most of the female applicants stop in the second round.

At this time, we cannot make any inference or conclusion on who is more likely being a successful candidate. After we fit models and assess relationships among different factors like GPA, work experience, and technical skills etc., we may make inferences at that time.

### Models

Table 1: Coefficient estimates for the logistic model built for hiring phase-I

```{r, warning=TRUE}
# build model
hire1_mod_wg <- glm(pass ~ gender + cv + gpa + extracurriculars + work_experience, family = binomial(), data = hire_phase1)
summary(hire1_mod_wg)
hire1_ci <- confint(hire1_mod_wg)

# table for reporting
ests_h1 <- format(round(exp(summary(hire1_mod_wg)$coef)[, 1], 2), nsmall = 2)
cis_h1 <- format(round(exp(hire1_ci), 6), nsmall = 2)
cis_h1 <- str_c("(", trimws(cis_h1[, 1]), ", ", cis_h1[, 2], ")")
pval_h1 <- format(round(summary(hire1_mod_wg)$coefficients[, 4], 4), 2)
hire1_rownames <- c("Baseline odds", "Gender: Prefer not to say", "Gender: Woman", "CV", "GPA", "Extracurriculars", "Work experience")
...
Table 2: Coefficient estimates for the logistic model built for hiring phase-II
...
hire2_ci <- confint(hire2_mod_wg)

# table for reporting
ests_h2 <- format(round(exp(summary(hire2_mod_wg)$coef)[, 1], 2), nsmall = 2)
cis_h2 <- format(round(exp(hire2_ci), 6), nsmall = 2)
cis_h2 <- str_c("(", trimws(cis_h2[, 1]), ", ", cis_h2[, 2], ")")
pval_h2 <- format(round(summary(hire2_mod_wg)$coefficients[, 4], 4), 2)
hire2_rownames <- c("Baseline odds", "Gender: Prefer not to say", "Gender: Woman", "Technical skills", "Writing skills", "Leadership_presence", "Speaking_skills")
